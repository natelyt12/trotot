...
export const useDashboard = (user, initialData, routerPage) => {
    ...
    const handlePublishFromDraft = useCallback(
        (room) => {
            const errors = validateRoomData(room);

            if (errors.length > 0) {
                const errorMessage =
                    "Không thể công khai tin đăng vì thiếu thông tin:\n" + errors.map((e) => "\n• " + e).join("") + "\n\nVui lòng bấm vào Sửa để bổ sung.";
                showModal({
            ...
            showModal({
                ...
                message:
                    "Bạn có chắc chắn muốn công khai tin đăng này? Hãy đảm bảo bạn đã kiểm tra kỹ các thông tin. Tin đăng của bạn sẽ được hệ thống kiểm duyệt trong vòng 24h trước khi hiển thị rộng rãi.",
                type: "warning",
