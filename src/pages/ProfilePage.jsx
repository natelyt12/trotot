...
/* ============================================
   ...
   ============================================ */
export default function ProfilePage({ user, navigate, initialData, currentPage }) {
    ...
    useEffect(() => {
        if (currentPage === "profile") {
            const targetTab = initialData?.tab || "info";
    ...
    const handleUpdateProfile = async (e) => {
        ...
        if (oldRole === "tenant" && newRole === "landlord") {
            ...
            showModal({
                ...
                message:
                    "Để chuyển đổi vai trò sang Bên cho thuê (Chủ nhà), bạn cần hoàn tất biểu mẫu xác thực danh tính (KYC). Bạn có muốn bắt đầu xác thực ngay bây giờ?",
                type: "warning",
                confirmText: "Bắt đầu xác thực",
                cancelText: "Hủy bỏ",
                onConfirm: () => {
            ...
            showModal({
                ...
                message:
                    "Khi chuyển về vai trò Người thuê, toàn bộ tin đăng của bạn trên hệ thống sẽ tự động chuyển thành bản nháp (tạm ẩn khỏi trang tìm kiếm) và bạn không thể quản lý tin đăng nữa. Bạn có chắc chắn muốn chuyển đổi?",
                type: "error",
    ...
    // Shared input class
    const inputCls =
        "w-full px-3 py-2.5 h-11 border border-stone-200 rounded-lg text-sm text-stone-900 outline-none focus:border-amber-500 transition-colors duration-200 bg-white";
