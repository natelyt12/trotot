...
export default function RoomPostForm({ user, onClear, onSuccess }) {
    ...
    useEffect(() => {
        const selectedProvince = PROVINCE.find(p => p.name === formData.city);
        if (selectedProvince) {
            setDistricts(selectedProvince.districts || []);
            // Reset district/ward if not found in new province
            if (!selectedProvince.districts.some(d => d.name === formData.district)) {
    ...
    useEffect(() => {
        ...
        if (selectedDistrict) {
            ...
            if (!selectedDistrict.wards.some(w => w.name === formData.ward)) {
        ...
        return new Promise((resolve) => {
            ...
            reader.onload = (event) => {
                ...
                img.onload = () => {
                    ...
                    // Xuất ra dạng Blob JPEG, chất lượng 0.7
                    canvas.toBlob((blob) => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            // Tính ngày hết hạn (1 tuần kể từ bây giờ)
            ...
            const payload = {
                ...
                monthly_costs: formData.monthly_costs,
                room_features: formData.room_features,
                rules_utilities: formData.rules_utilities,
                media_contact: {
            ...
            showModal({
                title: 'Thành công',
                message: 'Tin đăng của bạn đã được gửi và đang chờ duyệt!',
                type: 'success',
                onConfirm: () => {
            ...
            showModal({ title: 'Lỗi', message: 'Có lỗi xảy ra khi lưu tin. Vui lòng thử lại!', type: 'error' });
        } finally {
    ...
    return (
        <form onSubmit={handleSubmit} className="space-y-8 pb-10">
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-stone-100 pb-6">
                <div>
                    <h3 className="text-xl font-bold text-stone-900 font-heading">Đăng tin cho thuê mới</h3>
                ...
                <div className="flex items-center gap-3">
                    <button 
                        ...
                        className="px-5 py-2.5 rounded-full border border-stone-200 text-stone-600 font-bold text-sm hover:bg-stone-50 transition-colors cursor-pointer"
                    >
                        Hủy bỏ
                    </button>
                    <button 
                    ...
                    >
                        {isSubmitting ? (
                            ...
                            <><AppIcon name="plus" size={16} /> Đăng tin ngay</>
                        )}
                    </button>
                </div>
            </div>
            ...
            <div className="max-w-3xl mx-auto space-y-6">
                                        ...
                                        >
                                            ...
                                            <div className="absolute inset-0 bg-black/40 opacity-0 hover:opacity-100 transition-opacity flex flex-col justify-between p-1.5">
                                                <div className="flex justify-end">
                                                    <button type="button" onClick={() => removeImage(idx)} className="w-5 h-5 rounded-full bg-red-500 text-white flex items-center justify-center cursor-pointer border-none shadow-sm">
                                                        <AppIcon name="trash" size={10} />
                                                    </button>
                                                </div>
                                                <button type="button" onClick={() => setAsCover(idx)} className="w-full py-1 rounded-md text-[0.55rem] font-bold uppercase cursor-pointer border-none bg-white/90 text-stone-900">
                                                    {img.is_cover ? 'Bìa' : 'Làm bìa'}
                                                </button>
                                            </div>
                                            {img.is_cover && (
                                                <div className="absolute top-1 left-1 bg-amber-500 text-white text-[0.5rem] font-black uppercase px-1.5 py-0.5 rounded shadow-sm">Bìa</div>
                                            )}
                                        </motion.div>
                                    ))}
                                </AnimatePresence>
                    ...
                    <div className="space-y-4">
                        ...
                        {nearbyUniversities.length > 0 && (
                            <div className="mt-4 p-3 bg-blue-50/50 border border-blue-100 rounded-xl">
                                <div className="flex gap-2 items-start text-blue-600">
                                    ...
                                    <div>
                                        ...
                                        <p className="text-[0.75rem] font-medium leading-relaxed">
                                            {nearbyUniversities.map(u => u.name).join(', ')}
                                        </p>
                                    </div>
                                </div>
                            </div>
                ...
                <div className="pt-6 border-t border-stone-100 flex flex-col items-center">
                    ...
                    >
                        {isSubmitting ? 'Đang lưu tin...' : 'Đăng tin ngay'}
                    </button>
