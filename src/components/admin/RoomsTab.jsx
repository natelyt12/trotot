...
export default function RoomsTab({
    ...
    const baseRooms = (roomUserFilter
        ? allRooms.filter(r => 
            (r.owner || '').toLowerCase().includes(roomUserFilter.name.toLowerCase()) || 
            (r.owner_phone || '').includes(roomUserFilter.name)
          )
        : allRooms
    ).filter(r => {
    ...
    const currentList = 
        roomsSubTab === 'verified' 
            ...
            : roomsSubTab === 'unverified'
                ? unverifiedRooms
                : pendingPublishRooms; // Default is pending_publish
    ...
    return (
        <div className="space-y-5">
            ...
            <div className="flex border-b border-stone-200 overflow-x-auto whitespace-nowrap">
                ...
                >
                    Chờ duyệt công khai ({pendingPublishRooms.length})
                </button>
                <button
            ...
            {currentList.length === 0 ? (
                <div className="text-center py-20 bg-stone-50 border border-dashed border-stone-200 rounded-2xl">
                    ...
                    <p className="text-stone-500 font-bold text-sm">
                        {roomsSubTab === 'verified' ? 'Không có tin trọ nào đã xác thực' : 
                         roomsSubTab === 'unverified' ? 'Không còn phòng trọ nào chưa xác thực' :
                ...
                <div className="space-y-6">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        {paginatedRooms.map(room => (
                            <div key={room.id} className="flex flex-col p-4 border border-stone-200 rounded-xl hover:border-amber-400 transition-all bg-white group gap-3">
                                {/* Image & Core specs */}
                            ...
                            {(() => {
                                ...
                                return pages.map((pageNum) => (
                                    <button
                                        ...
                                        className={`w-8 h-8 flex items-center justify-center rounded-full border text-xs font-bold transition-colors cursor-pointer ${currentPage === pageNum ? 'border-amber-500 bg-amber-50 text-amber-600' : 'border-stone-200 text-stone-600 hover:bg-stone-50'
                                            }`}
                                    >
                                        {pageNum}
                                    </button>
