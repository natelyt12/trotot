            ...
            <div className="relative overflow-hidden w-[130px] sm:w-full shrink-0 sm:h-[200px] bg-stone-100 flex items-center justify-center">
                ...
                <button
                    ...
                    className={`absolute bottom-2 left-2 sm:top-2 sm:right-2 sm:bottom-auto sm:left-auto w-8 h-8 rounded-full flex items-center justify-center transition-all duration-200 z-5 border-none cursor-pointer shadow-sm ${
                        favorited ? "bg-red-500 text-white" : "bg-white/80 text-stone-600 hover:bg-white hover:text-red-500"
                    }`}
                    title={favorited ? "Bỏ lưu tin" : "Lưu tin"}
                >
                    <AppIcon name="heart" size={16} fill={favorited ? "currentColor" : "none"} strokeWidth={2.5} />
                </button>
                ...
                <div className="flex flex-col gap-2 mt-auto">
                    ...
                    <div className="flex items-center gap-1 text-stone-400">
                        {room_features.amenities.slice(0, 5).map((key) => (
                            <div key={key} title={AMENITIES[key]?.label} className="w-6 h-6 rounded-full bg-stone-100 flex items-center justify-center">
